...
packages <- c("ks", "here", "plotrix", "lattice", "adehabitatHR", "mapview", "ggplot2","colorRamps", "sf", "terra", "tmap", "stars", "dplyr")
```
...
```{r}
par(mfrow = c(2,3))
hist(wolfused$DEER_W) ### wolves seems to use medium to high habitat suitability indexes for deer
hist(wolfused$ELK_W)
...
bwplot(WOLF_W~pack, auto.key=TRUE,allow.multiple = TRUE,data=wolfused, outer=TRUE)
```
